...
interface ClickablePieceProps {
  piece: GamePieceType
}

const ClickablePiece: React.FC<ClickablePieceProps> = ({ piece }) => {
  const { transformPiece, placePiece } = useGameStore()
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    switch (e.key.toLowerCase()) {
      ...
      case ' ':
        e.preventDefault()
        // Try to place at 0,0 for testing
        placePiece(piece, 0, 0)
        break
    }
  }
  ...
  return (
    <div
      ...
      tabIndex={0}
      role="button"
      aria-label={`Clickable piece: ${piece.shape}, value ${piece.value}. Press R to rotate, F to flip, Enter to place`}
    ...
    <div className="bg-white rounded-cozy-lg shadow-cozy-lg p-6 border-2 border-purple-200">
          ...
          return (
            <div key={shape} className="space-y-2">
              ...
              <div className="flex flex-wrap gap-3 p-3 bg-purple-50 rounded-cozy">
                {pieces.map(piece => (
                  <ClickablePiece key={piece.id} piece={piece} />
                ))}
              </div>
            </div>
          )
      ...
      <div id="piece-controls" className="mt-6 p-3 bg-pastel-purple-100 rounded-cozy text-sm text-purple-700">
        ...
        <div className="space-y-1 text-xs">
          ...
          <div><kbd className="px-1 py-0.5 bg-white rounded text-purple-800">Enter</kbd> - Place at (0,0)</div>
          <div className="opacity-75">Click pieces to place at (0,0) for testing</div>
        </div>
      </div>
